...
pub struct Uart{
    // the UART base address at the I/O memory address space
    base_address: usize
}
...
impl Uart {
    ...
    // get the configurations correct
	pub fn init(&mut self) {
		let ptr = self.base_address as *mut u8;
		unsafe {
			// setting the word length, which
			...
			// enable the FIFO, which is bit index 0 of the FIFO
	...
	pub fn fill_uart_buffer(&mut self, c: u8) {
		let ptr = self.base_address as *mut u8;
		unsafe {
	...
	pub fn read_uart_buffer(&mut self) -> Option<u8> {
		let ptr = self.base_address as *mut u8;
		unsafe {
...
// implement the write_str for Uart
impl Write for Uart {
	fn write_str(&mut self, out: &str) -> Result<(), Error> {
		for c in out.bytes() {
			self.fill_uart_buffer(c);
		}
		Ok(())
	}
}